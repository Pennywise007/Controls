...
BOOL CEditBase::PreTranslateMessage(MSG* pMsg)
{
    ...
    // провер€ем не используютс€ ли комбинации клавиш
...
BOOL CEditBase::CheckNumericString(_In_ CString& Str)
{
    ...
    // букв "е" или "≈" не должно быть бьльше 1
    if (CountElements(Str, L'e') + CountElements(Str, L'E') > 1)
        return ShowError();

    // знаков отрицани€ не должно быть больше 2
    if (CountElements(Str, L'-') > 2)
        return ShowError();

    // проходим по всему тексту провер€€ каждый символ
        ...
        if (!iswdigit(ch))	// если вводим не цифры
        {
            ...
            case 45:	// минус
                ...
                if ((m_bUsePositivesDigitsOnly && i == 0) || ((i != 0) && (Str[i - 1] != L'e' && Str[i - 1] != L'E')))
                    return ShowError();
                break;
            case 101:	// "e" английска€ дл€ 1.2e-3
            case 69:	// "E" английска€ дл€ 1.2E-3
                // E/e не могут сто€ть с начала
                if (i == 0)
                    return ShowError();
                break;
            default:
...
afx_msg LRESULT CEditBase::OnPaste(WPARAM wParam, LPARAM lParam)
{
    ...
    // формируем новую строку вставл€€ в нее текст из буфера
